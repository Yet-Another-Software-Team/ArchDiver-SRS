...
\usepackage{placeins}  % Ensure floats do not move past FloatBarriers / sections
\usepackage{pdflscape}
\usepackage{adjustbox}  % For resizing images in landscape pages
\usepackage{pgfgantt} % gantt chart maker
\usepackage{xcolor} 
...
\def \srsTitle{ArchDiver: Software architecture smell analysis and explanation using GraphSAGE and LLM}
\def \srsNickName{ArchDiver}
\def \srsNickname{\srsNickName} % for backward compatibility and ease of use
\def \srsAuthorOne{Sirapob Phasut}
\def \srsAuthorTwo{Khunakorn Pattayakorn}
\def \srsAuthorThree{}
\def \srsAcademicYear{2568}

\def \srsAdvisorName{Asst. Prof. Paruj Ratanaworabhan}
\def \srsCoAdvisorName{}
\def \srsHoDName{Assoc. Prof. Punpiti Piamsa-nga}
%% -----------------------------------
